...
export default {
				...
				if (res.flag) {
					...
					if (this.clusterOptions.length > 0) {
						...
						for (const index in this.clusterOptions) {
							this.grafanaMap.set(this.clusterOptions[index].clusterId, this.clusterOptions[index].extension.grafana.server)
						}
					}
					this.getUrl()
				} else {
	...
	// 	getUrl() {
	// 		const that = this
	// 		that.srcUrl = $themeConfig.app.grafanaHttpUrl + "/d/9CWBz0bik/cluster-dashboard?refresh=10s&orgId=1&kiosk";
	// 	}
	// }
}
</script>
